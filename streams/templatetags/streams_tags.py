...
@register.simple_tag(name='lst_maker')
def lst_maker(length):
    return list(range(length))


@register.simple_tag(name='int_to_word')
def int_to_word(numb):
	p = inflect.engine()
	return p.number_to_words(numb).capitalize()
...
@register.simple_tag(name='box_or_blank')
def box_or_blank(a, b):
    if len(a) == 1:
        lst = [None, a[0], None]
    elif len(a) == 2:
        lst = [a[0], None, a[1]]
